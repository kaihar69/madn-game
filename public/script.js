...
const joinBtn = document.getElementById('joinBtn');
const startBtn = document.getElementById('startWithBotsBtn');
const leaveBtn = document.getElementById('leaveBtn'); // NEU
const nameInput = document.getElementById('playerNameInput');
const cubeElement = document.getElementById('diceCube');
let myColor = null;
let amIPlaying = false;
...
leaveBtn.addEventListener('click', () => {
    if(confirm("Möchtest du das Spiel wirklich verlassen?")) {
        window.location.reload(); // Einfach Seite neu laden
    }
});
...
socket.on('serverStatus', (info) => {
    if (amIPlaying) {
        joinBtn.style.display = 'none'; nameInput.style.display = 'none'; 
        leaveBtn.style.display = 'inline-block'; // Zeigen wenn ich spiele
    ...
    leaveBtn.style.display = 'none'; // Verstecken wenn Zuschauer
    joinBtn.style.display = 'inline-block'; nameInput.style.display = 'inline-block';
...
socket.on('joinSuccess', (data) => {
    ...
    leaveBtn.style.display = 'inline-block'; // Zeigen
});
...
// ... REST WIE GEHABT (gameStarted, updateBoard, diceRolled...) ...
// ... Bitte den Rest aus V2.1 übernehmen (copy paste) ... 
