...
// CHECK: Bin ich verbunden?
socket.on('connect', () => {
    console.log("Verbunden mit ID:", socket.id);
    document.getElementById('log-container').innerText = "Verbunden!";
});
...
joinBtn.addEventListener('click', () => {
    // Feedback für User
    document.getElementById('log-container').innerText = "Sende Anfrage...";
...
leaveBtn.addEventListener('click', () => {
    if(confirm("Spiel verlassen?")) window.location.reload();
});

// --- LOBBY LOGIK ---
socket.on('serverStatus', (info) => {
    if (amIPlaying) {
        joinBtn.style.display = 'none'; nameInput.style.display = 'none'; leaveBtn.style.display = 'inline-block';
        if (!info.running) { startBtn.style.display = 'inline-block'; startBtn.innerText = `Start (${info.count}/4)`; } 
        else { startBtn.style.display = 'none'; }
        return;
    }
    startBtn.style.display = 'none'; leaveBtn.style.display = 'none'; joinBtn.style.display = 'inline-block'; nameInput.style.display = 'inline-block';
...
socket.on('joinSuccess', (data) => {
    ...
    startBtn.style.display = 'inline-block'; leaveBtn.style.display = 'inline-block';
});

socket.on('sessionLost', () => { alert("Verbindung neu aufgebaut."); window.location.reload(); });
socket.on('joinError', (msg) => { alert(msg); document.getElementById('log-container').innerText = msg; });
